...
CBaseAuthorization<void> g_AlarmAuthorization;///<The alarm output authorizataion management
...
CDriverAlarm* CDriverAlarm::Instance()
{
	static CDriverAlarm DriverAlaram;
	return &DriverAlaram;
}
...
void CDriverAlarm::Output(BOOL bClearFunctionName)
{
	...
	///<Apply output auzhorization
	g_AlarmAuthorization.Apply();
...
int CDriverAlarm::OutputAlarm()
{
	...
	if ((USHORT)-1 == m_uSiteNo)
	{
		...
		if (0 == uSiteNum)
		{
			nTempSiteNum = DCM400_MAX_SITE_COUNT;
		}
	}
	else
	{
...
{
	bool ret = false;
	BYTE abySiteStatus[SYS_MAX_SITE_NUM] = { 0 };
