...
for (auto bySlot : setSlot){\
	for (auto byController : setController){\
	 usControllerID = GetControllerID(bySlot, byController);
...
{
	USHORT usControllerID = GetControllerID(bySlotNo, byControllerIndex);
	auto iterController = m_mapAverageData.find(usControllerID);
	if (m_mapAverageData.end() != iterController)
	{
		return -1;
	}

	m_mapAverageData.insert(make_pair(usControllerID, pusAverageData));
	return 0;
}
...
{
	USHORT usControllerID = GetControllerID(bySlotNo, byControllerIndex);
	auto iterController = m_mapSampleSetting.find(usControllerID);
	if (m_mapSampleSetting.end() != iterController)
	{
		return -1;
	}
	m_mapSampleSetting.insert(make_pair(usControllerID, SAMPLE_SETTING()));
	iterController = m_mapSampleSetting.find(usControllerID);
...
{
	USHORT usControllerID = GetControllerID(bySlotNo, byControllerIndex);
	auto iterController = m_mapMeasureMode.find(usControllerID);
	if (m_mapMeasureMode.end() != iterController)
	{
		return -1;
	}
	m_mapMeasureMode.insert(make_pair(usControllerID, pucMeasureType));
	return 0;
}
...
{
	USHORT usControllerID = GetControllerID(bySlotNo, byControllerIndex);
	auto iterController = m_mapLatestMeasureMode.find(usControllerID);
	if (m_mapLatestMeasureMode.end() != iterController)
	{
		return -1;
	}
	m_mapLatestMeasureMode.insert(make_pair(usControllerID, pucMeasureType));
	return 0;
}
...
{
	USHORT usControllerID = GetControllerID(bySlotNo, byControllerIndex);
	auto iterController = m_mapForceMode.find(usControllerID);
	if (m_mapForceMode.end() != iterController)
	{
		return -1;
	}

	m_mapForceMode.insert(make_pair(usControllerID, FORCE_MODE()));
	iterController = m_mapForceMode.find(usControllerID);
...
// 		return -1;
// 	}
// 	ULONG ulAD7606[DCM400_CHANNELS_PER_CONTROL / 2] = { 0 };
...
// 	UINT usControllerID = GetControllerID(bySlotNo, byControllerIndex);
// 	auto iterData = m_mapAverageData.find(usControllerID);
...
// 
// 	USHORT usChannel = 0;
// 	USHORT usChannelCount = DCM400_CHANNELS_PER_CONTROL / 2;
...
{
	USHORT usControllerID = 0;
	BIND_SET_BEIGIN(bySlotNo, byController, vecChannel)
	{
		auto iterLatestMeasMode = m_mapLatestMeasureMode.find(usControllerID);
		auto iterCurMeasMode = m_mapMeasureMode.find(usControllerID);

		auto iterSample = m_mapSampleSetting.find(usControllerID);
...
void CPMU::SetMeasureMode(BYTE bySlotNo, BYTE byController, const std::vector<USHORT>& vecChannel, unsigned char ucMeasureMode)
{
	...
	BIND_SET_BEIGIN(bySlotNo, byController, vecChannel)
	{
		auto iterControllerID = m_mapLatestMeasureMode.find(usControllerID);
...
int CPMU::SetForceMode(BYTE bySlotNo, BYTE byController, const std::vector<USHORT>& vecChannel, unsigned char ucForceMode, unsigned char ucIRange)
{
	...
	BIND_SET_BEIGIN(bySlotNo, byController, vecChannel)
	{
		auto iterControllerID = m_mapForceMode.find(usControllerID);
...
inline BYTE CPMU::GetControllerID(BYTE bySlotNo, BYTE byControllerIndex)
{
