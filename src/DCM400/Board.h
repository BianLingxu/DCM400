...
public:
	...
	*/
	 ...
	 * - -3 No channel existed
	*/
	int SetPeriodSeries(const std::vector<USHORT>& vecChannel, USHORT usSeriesIndex, double dPeriod);
	/**
	 * @brief Set the series edge
	 * @param[in] vecChannel The channel number
	 * @param[in] usSeriesIndex The series index
	 * @param[in] pdEdge The each edge's value
	 * @return Execute result
	 * - 0 Set the edge successfully
	 * - -1 The series is over range
	 * - -2 The point pointed to edge is nullptr
	 * - -3 Not channel existed
	*/
	int SetEdgeSeries(const std::vector<USHORT>& vecChannel, USHORT usSeriesIndex, const double* pdEdge);
	/**
	 * @brief Set the series format
	 * @param[in] vecChannel The channel number
	 * @param[in] usSeriesIndex The series index
	 * @param[in] WaveFormat The wave format
	 * @param[in] IOFormat The IO formate
	 ...
	 * - -3 Not channel existed
	*/
	int SetFormatSeries(const std::vector<USHORT>& vecChannel, USHORT usSeriesIndex, const WAVE_FORMAT& WaveFormat, const IO_FORMAT& IOFormat, const COMPARE_MODE& CompareMode);
	/**
	 * @brief Set the time sest
	 * @param[in] vecChannel The channel number
	 * @param[in] usTimesetIndex The time set index
	 * @param[in] byPeriodSeries The period series
	 * @param[in] pbyEdgeSeries The edge series
	 * @param[in] byFormatSeries The format series
	 * @return Execute result
	 * - 0 Set the time set successfully
	 * - -1 The time set is over range
	 * - -2 The series is over range
	 * - -3 The point pointed to edge series is nullptr
	 * - -4 No channel existed
	*/
	int SetTimeSet(const std::vector<USHORT>& vecChannel, USHORT usTimesetIndex, BYTE byPeriodSeries, const BYTE* pbyEdgeSeries, BYTE byFormatSeries);
	/**
	 ...
	 * - -3 The pattern number is over range
	 * - -4 Allocate memory fail
	*/
	int SetVector(USHORT usChannel, UINT uPatternLine, char cPattern, const CPatternCMD& PatternCMD);
	/**
	 ...
	 * @param[in] vecChannel The channel of the board
	 * @return Whether the vector is valid
	 * - TRUE The vector is valid
	 * - FALSE The vector is invalid 
	 */
	BOOL IsVectorValid(const std::vector<USHORT>& vecChannel);
	 /**
	 ...
	 * - -3 No valid channel
	 */
	int SetOperand(const std::vector<USHORT>& vecChannel, UINT uLineNo, USHORT usOperand, BOOL bCheckRange);
	/**
	...
	*/
	int SetInstruction(const std::vector<USHORT>& vecChannel, UINT uLineNo, const char* lpszInstruction, USHORT usOperand);
	/**
	 ...
	 * @param[in] uRAMLineNo The RAM line number
	...
	*/
	int SetSaveSelectFail(const std::vector<USHORT>& vecChannel, UINT uRAMLineNo, BOOL bStartSave, BOOL bDelete = FALSE);
	/**
	...
	*/
	int GetInstruction(BYTE byController, UINT uLineNo, char* lpszInstruction, int nBuffSize);
	/**
	 ...
	 * - -3 The line number is over range
	*/
	int GetOperand(BYTE byController, UINT uLineNo);
	/**
	...
	*/
	 ...
	 * @param[in] usChannel The channel number
	 * @param[out] nLineNo The last line number certail
	 * @param[out] bLineFail The result of last certained line number
	 * @return execute result
	 * - 0 Get the certain line successfully
	 * - -1 The channel number is over range
	 * - -2 The controller is not existed
	*/
	int GetLastCertainResultLineNo(USHORT usChannel, int& nLineNo, BOOL& bLineFail);
	/**
	...
	* - -4 Vector running
	*/
	int GetCapture(USHORT usChannel, std::vector<BYTE>& vecCapture);
	/**
	...
	*/
	int SetCalibrationData(BYTE byControllerIndex, DCM400_CAL_DATA* pCalibrationData, int nElementCount);
	/**
	...
	*/
	int GetCalibrationData(BYTE byControllerIndex, DCM400_CAL_DATA* pCalibrationData, int nElementCount);
	/**
	...
	*/
	 ...
	 */
	int SetPrereadLine(const std::vector<USHORT>& vecChannel, UINT uStartLine, UINT uLineCount);
	/**
	...
	*/
	int SetLineInfo(const std::vector<USHORT>& vecChannel, UINT uStartLine, UINT uLineCount);
	/**
	...
	*/
	int GetVector(BYTE byControllerIndex, UINT uStartLine, UINT uLineCount, char(*lpszPattern)[17]);
	/**
	...
	*/
	int GetMemory(BYTE byControllerIndex, UINT uStartLine, UINT uLineCount, USHORT* pusData);
	/**
	...
	*/
	int SetMemory(USHORT usChannel, UINT uStartLine, UINT uLineCount, BYTE* pbyData);
	/**
	...
	*/
	int GetVector(USHORT usChannel, UINT uStartLine, UINT uLineCount, char*lpszPattern);
	/**
...
private:
	...
	CDriverAlarm* m_pAlarm;///<The point pointed to alarm class
	...
	std::map<BYTE, ULONG> m_mapFailNoFilter;///<The controller whose MCU result have be filtered
	std::set<BYTE> m_setPatternController;///<The controller load vector
};
