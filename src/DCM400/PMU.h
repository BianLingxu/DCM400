...
private:
	...
	*/
	inline BYTE GetControllerID(BYTE bySlotNo, BYTE byControllerIndex);
private:
	/**
	...
	*/
	...
	std::map<BYTE, USHORT*> m_mapAverageData;///<The PMU data of each controller, the key is controller ID and the value is PMU data
	std::map<BYTE, SAMPLE_SETTING> m_mapSampleSetting;///<The sample setting of each channel
	std::map<BYTE, UCHAR*> m_mapMeasureMode;///<The measure mode of each channel;
	std::map<BYTE, UCHAR*> m_mapLatestMeasureMode;///<The measurement mode of each channel in latest set
	std::map<BYTE, FORCE_MODE> m_mapForceMode;///<The force mode of each channel
};
