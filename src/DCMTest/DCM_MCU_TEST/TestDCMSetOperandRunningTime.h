/**
 * @file TestDCMSetOperandRunningTime.h
...
XT_TEST(FunctionRunningTimeTest, TestDCMSetOperandRunningTime)
{
	string strFuncName;
	...
	dcm.SetInstruction("G_ALLPIN", "TEST_INS_ST", 1, "JUMP", "TEST_INS_ST");
	timeReport.timeStart();
	dcm.SetOperand("G_ALLPIN", "TEST_INS_ST", 1, "TEST_INS_SP");
	timeReport.timeStop();
	timeReport.addMsg("Set label operand to %d controllers", nControllerCount);

	dcm.SetInstruction("G_ALLPIN", "TEST_INS_ST", 1, "REPEAT", "1");
	timeReport.timeStart();
	dcm.SetOperand("G_ALLPIN", "TEST_INS_ST", 1, "10");
	timeReport.timeStop();
	timeReport.addMsg("Set number operand to %d controllers", nControllerCount);
