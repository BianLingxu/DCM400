#pragma once
/*!
* @file      TestDCMInitMCURunningTime.h
*
...
XT_TEST(FunctionRunningTimeTest, TestDCMInitMCURunningTime)
{
	string strFuncName;
	...
	timeReport.timeStop();
	timeReport.addMsg("Initialization %d channels", nChannelCount);

	timeReport.timeStart();
		dcm.InitMCU("G_EVENPIN");
	timeReport.timeStop();
	timeReport.addMsg("Initialization %d channels of even channel", nChannelCount / 2);

	timeReport.timeStart();
	dcm.InitMCU("G_ODDPIN");
	timeReport.timeStop();
	timeReport.addMsg("Initialization %d channels of odd channel", nChannelCount / 2);
