#pragma once
/*!
* @file      TestDCMStopVectorRunningTime.h
*
...
* @date      2017/12/12
* @version   v 1.0.0.0
* @brief     测试StopVector运行时间
* @comment
*/
#include "..\DCMTestMain.h"
XT_TEST(FunctionRunningTimeTest, TestDCMStopVectorRunningTime)
{
	string strFuncName;
	...
	timeReport.timeStop();
	timeReport.addMsg("Stop Vector: %d channels ", nChannelCount);

	timeReport.timeStart();
	dcm.StopVector("G_ODDPIN");
	timeReport.timeStop();
	timeReport.addMsg("Stop Vector: %d channels of odd pin ", nChannelCount / 2);

	timeReport.timeStart();
	dcm.StopVector("G_EVENPIN");
	timeReport.timeStop();
	timeReport.addMsg("Stop Vector: %d channels of even pin ", nChannelCount / 2);
