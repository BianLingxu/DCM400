#pragma once
/**
 * @file TestDCMI2CWriteData_SameFunction.h
 * @brief Check the function of I2CWriteData
 * @author Guangyun Wang
 * @date 2021/08/02
 * @copyright AccoTEST Business Unit of Beijing Huafeng Test & Controller Technology Co., Ltd.
*/
#include "..\DCMTestMain.h"
XT_TEST(FunctionFunctionTest, TestDCMI2CWriteData_SameFunction)
{
	string strFuncName;
	...
	BYTE abyDataWritten[nMaxDataLength] = { 0xAA,0x55,0xA5,0x5A,0x0F };
	...
	for (int nTestIndex = 0; nTestIndex < nTestCount; ++nTestIndex)
	{
		...
		for (int nDataTestIndex = 0; nDataTestIndex < nDataTestCount; ++nDataTestIndex)
		{
			...
			for (int nREGModeIndex = 0; nREGModeIndex < nREGModeTestCount; ++nREGModeIndex)
			{
				...
				if (0 == nTestIndex)
				{
					funcReport.AddTestItem("I2C: BRAM. REG size:%d bits. Data: %d bytes.", nREGByte[nREGModeIndex], nCurDataLength);
				}
				else
				{
					if (!bBRAMOccupy)
					...
					funcReport.AddTestItem("I2C: DRAM. REG size:%d bits. Data: %d bytes.", nREGByte[nREGModeIndex], nCurDataLength);


					//Firstly, generate the I2C pattern vector.
					dcm.I2CWriteData(bySlaveAddr, nREGAddr, nCurDataLength, abyDataWritten);
					...
					for (USHORT usSiteNo = 0; usSiteNo < usSiteCount; ++usSiteNo)
					{
						...
						for (int nDataIndex = 0; nDataIndex < nCurDataLength; ++nDataIndex)
						{
							...
							nOffset = dcm_GetLabelLineNo(lpszStartLabel) - nRAMOffset;
							byRealData = GetData(nOffset, 8, &nSDADataFailIndex, nSDADataFailCount, puSDADataFailLineNo);
							byExpectData = abyDataWritten[nDataIndex];
							XT_EXPECT_EQ(byRealData, byExpectData);
							if (byRealData != byExpectData)
							{
	...
	dcm.I2CWriteData(0x20, 0x00, 4, abyDataWritten);
	RestoreSite();
