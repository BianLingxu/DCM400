#pragma once
/*!
* @file      TestDCMRunVectorWithGroupParamValidity.h
*
...
* @date      2017/09/27
* @version   v 1.0.0.0
* @brief     测试RunVectorWithGroup参数有效性
* @comment
*/
#include "..\DCMTestMain.h"

XT_TEST(ParamValidityTest, TestDCMRunVectorWithGroupParamValidity)
{
	string strFuncName;
	...
	nRetVal = dcm.RunVectorWithGroup("G_ALLPIN", "FRIST_ST", "FRIST_SP");
	...
	{
		nRetVal = dcm.RunVectorWithGroup(lpszPinGroup[nTestIndex], "FIRST_ST", "FIRST_SP");
	...
	for (int nStartLabelIndex = 0; nStartLabelIndex < nTestCount; ++nStartLabelIndex)
	{
		...
		{
			nRetVal = dcm.RunVectorWithGroup("G_ALLPIN", lpszLabel[nStartLabelIndex], lpszLabel[nStopLabelIndex]);
