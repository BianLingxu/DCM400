#pragma once
/*!
* @file      TestDCMGetFailLineNoRunningTime.h
*
...
* @date      2017/11/15
* @version   v 1.0.0.0
* @brief     测试GetFailLineNo运行时间
* @comment
*/

#include "..\DCMTestMain.h"
XT_TEST(FunctionRunningTimeTest, TestDCMGetFailLineNoRunningTime)
{
	string strFuncName;
	...
	dcm.GetFailLineNo("CH0", iterSlot->second, 100, ulFailLineNo);
	timeReport.timeStop();
	ULONG ulLineCount = 0;
	...
	dcm.GetFailCount("CH0", iterSlot->second, ulFailCount);
	timeReport.addMsg("%d lines vector without DRAM line.The fail count is %d.", ulLineCount, ulFailCount);
	...
	dcm.GetFailCount("CH0", iterSlot->second, ulFailCount);
	timeReport.addMsg("%d lines vector with DRAM line. The fail count is %d.", ulLineCount, ulFailCount);
	//dcm.SaveFailMap(0);
