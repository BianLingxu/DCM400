#pragma once
/*!
* @file      TestDCMWriteWaveData_BlankParamValidity.h
*
...
* @date      2020/01/06
* @version   v 1.0.0.0
* @brief     测试TestDCMWriteWaveData_BlankParamValidity参数有效性
* @comment
*/
#include "..\DCMTestMain.h"
XT_TEST(ParamValidityTest, TestDCMWriteWaveData_BlankParamValidity)
{
	string strFuncName;
		...
		for (int nCountIndex = 0; nCountIndex < nCountTestCount; ++nCountIndex)
		{
			...
			if (nMaxLineCount <= nLabelLine + ulOffset[nOffsetIndex])
			{
				if (OFFSET_ERROR != nRetVal)
				{
					...
					errMsg.SaveErrorMsg("No warning when the offset plus count is overrange, the return value is %d!", nRetVal);
				}
			}
