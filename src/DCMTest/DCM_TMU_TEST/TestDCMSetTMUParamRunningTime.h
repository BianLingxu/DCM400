#pragma once
/**
 * @file TestDCMSetTMUParamRunningTime.h
 * @brief Test the running time of function SetTMUParam
...
XT_TEST(TMURunningTimeTest, TestDCMSetTMUParamRunningTime)
{
	string strFuncName;
	...
	for (int nIndex = 0; nIndex < 100; ++nIndex)
	{
		dcm.SetTMUParam("CH0", DCM_ALLSITE, DCM_POS, 0, 0);
	}
	timeReport.timeStop();
	...
	for (int nIndex = 0; nIndex < 100; ++nIndex)
	{
		dcm.SetTMUParam("CH0", mapSlot.begin()->first, DCM_POS, 0, 0);
	}
	timeReport.timeStop();
