#pragma once
/**
 * @file TestDCMInitPPMURunningTime.h
 * @copyright AccoTEST Business Unit of Beijing Huafeng Test & Controller Co., Ltd.
 * @author Guangyun Wang
 * @date 2020/07/17
 * @version v 1.0.0.0
 * @brief Test the execution time of InitPPMU
 */
#include "..\DCMTestMain.h"

XT_TEST(PMURunningTimeTest, TestDCMInitPPMURunningTime)
{
	string strFuncName;
	...
	timeReport.timeStop();
	timeReport.addMsg("Initialize %d channels' PMU", nChannelCount);
	
	timeReport.timeStart();
	dcm.InitPPMU("G_ODDPIN");
	timeReport.timeStop();
	timeReport.addMsg("Initialize %d channels' PMU", nChannelCount / 2);

	timeReport.timeStart();
	dcm.InitPPMU("G_EVENPIN");
	timeReport.timeStop();
	timeReport.addMsg("Initialize %d channels' PMU", nChannelCount / 2);
