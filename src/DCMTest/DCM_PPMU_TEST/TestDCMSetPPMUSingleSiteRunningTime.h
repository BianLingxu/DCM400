#pragma once
/**
 * @file TestDCMSetPPMUSingleSiteRunningTime.h
 * @copyright AccoTEST Business Unit of Beijing Huafeng Test & Controller Co., Ltd.
 * @author Guangyun Wang
 * @date 2020/07/17
 * @version v 1.0.0.0
 * @brief Test the execution time of SetPPMUSingleSite
 */
#include "..\DCMTestMain.h"

XT_TEST(PMURunningTimeTest, TestDCMSetPPMUSingleSiteRunningTime)
{
	string strFuncName;
	...
	dcm.SetPPMUSingleSite("G_ALLPIN", usSiteNo, DCM_PPMU_FVMV, 0, DCM_PPMUIRANGE_32MA);
	timeReport.timeStop();
	timeReport.addMsg("Set PMU to FVMV 0V, %d channels in SITE_%d", nChannelCount, 1);

	timeReport.timeStart();
	dcm.SetPPMUSingleSite("G_ODDPIN", usSiteNo, DCM_PPMU_FVMV, 0, DCM_PPMUIRANGE_32MA);
	timeReport.timeStop();
	timeReport.addMsg("Set PMU to FVMV 0V, %d channels in SITE_%d", nChannelCount / 2, 1);

	timeReport.timeStart();
	dcm.SetPPMUSingleSite("G_EVENPIN", usSiteNo, DCM_PPMU_FVMV, 0, DCM_PPMUIRANGE_32MA);
	timeReport.timeStop();
	timeReport.addMsg("Set PMU to FVMV 0V, %d channels in SITE_%d", nChannelCount / 2, 1);
