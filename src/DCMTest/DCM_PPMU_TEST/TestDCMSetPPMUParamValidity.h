#pragma once
/**
 * @file TestDCMSetPPMUParamValidity.h
 * @copyright AccoTEST Business Unit of Beijing Huafeng Test & Controller Co., Ltd.
 * @author Guangyun Wang
 * @date 2020/07/16
 * @version v 1.0.0.0
 * @brief Test the parameter validity of SetPPPMU
 */
#include "..\DCMTestMain.h"
XT_TEST(PMUParamValidityTest, TestDCMSetPPMUParamValidity)
{
	string strFuncName;
	...
	{
		nRetVal = dcm.SetPPMU(lpszPinGroup,  DCM_PPMU_FVMV, 0, DCM_PPMUIRANGE_32MA);
	...
	for (BYTE byModeIndex = 0; byModeIndex < 5; ++byModeIndex)
	{
		for (BYTE bySetVoltageIndex = 0; bySetVoltageIndex < byTestVoltageCount; ++bySetVoltageIndex)
		{
			...
			nRetVal = dcm.SetPPMU("G_ALLPIN", PPMUMode(byModeIndex), dSetValue, DCM_PPMUIRANGE_32MA);
	...
	for (BYTE byRangeIndex = 0; byRangeIndex <= DCM_PPMUIRANGE_32MA + 1; ++byRangeIndex)
	{
		...
		{
			nRetVal = dcm.SetPPMU("G_ALLPIN", DCM_PPMU_FIMV, dCurrent, PPMUIRange(byRangeIndex));
			XT_EXPECT_EQ(nRetVal, 0);
			if (0 != nRetVal)
			{
				errMsg.SetErrorItem("Mode");
