...
CATE305::CATE305(BYTE bySlotNo)
	: CRegister(bySlotNo)
{
	SetModuleType(MODULE_TYPE::FUN_MODULE);
}

ULONG CATE305::GetStatus()
{
	return CRegister::Read(STATUS_ADDR);
}

int CATE305::Read(BYTE byAddress, std::map<USHORT, ULONG>& mapChannelData)
{
...
int CATE305::Write(BYTE byAddress, std::map<USHORT, ULONG>& mapChannelData)
{
	if (0 == mapChannelData.size())
	{
		return -2;
...
int CATE305::SetAddress(ULONG ulAddress)
{
	return -1;
}

int CATE305::SetAddress(int nType, USHORT usAddress)
{
	return -1;
}

int CATE305::SetStartLineNo(int nType, ULONG ulStartLineNo)
{
	return -1;
}

ULONG CATE305::Read(int nAddress)
{
	return -1;
}

int CATE305::Write(int nAddress, ULONG ulData)
{
	return -1;
}

int CATE305::Read(UINT uReadDataCount, ULONG* pulDataBuff)
{
	return -1;
}

int CATE305::Write(UINT uWriteDataCount, const ULONG* pulData)
{
	return -1;
}

int CATE305::Read(USHORT usAddress, UINT uReadDataCount, ULONG* pulDataBuff)
{
	return -1;
}

int CATE305::Write(USHORT usAddress, UINT uWriteDataCount, const ULONG* pulData)
{
	return -1;
}

inline ULONG CATE305::GetCTRLData(BYTE byAddress, USHORT uEnableChannel, BOOL bRead, BOOL bSame)
{
	BYTE byLow3Bit = bRead ? 1 : (bSame ? 3 : 7);
	return (uEnableChannel << 16) | (byAddress << 3) | byLow3Bit;
}

ULONG CATE305::ReadData(BYTE byAddress, BYTE byChannel)
{
...
void CATE305::WriteData(BYTE byAddress, BYTE byChannel, USHORT uData)
{
