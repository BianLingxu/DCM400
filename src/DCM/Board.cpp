...
int CBoard::IsChannelExisted(USHORT usChannel)
{
	...
	if (m_mapController.end() != m_mapController.find(usChannel / DCM_CHANNELS_PER_CONTROL))
	{
		return 1;
	}
	return 0;
...
int CBoard::SetTimesetDelay(BYTE byControllerIndex, double dTimesetDelay)
{
	if (DCM_MAX_CONTROLLERS_PRE_BOARD <= byControllerIndex)
	{
		return -1;
	...
	int nRetVal = iterController->second->SetTimesetDelay(dTimesetDelay);
	if (0 != nRetVal)
	{
		nRetVal = -3;
	}
	...
	{
		iterController->second->UpdateDelay();
	}
	return nRetVal;
}

double CBoard::GetTimesetDelay(BYTE byControllerIndex)
{
	if (DCM_MAX_CONTROLLERS_PRE_BOARD <= byControllerIndex)
	{
		return 1e16 - 1;
	...
	return iterController->second->GetTimesetDelay();
}

int CBoard::SetTotalStartDelay(BYTE byControllerIndex, double dDelay)
{
	if (DCM_MAX_CONTROLLERS_PRE_BOARD <= byControllerIndex)
	{
		return -1;
	...
	int nRetVal = iterController->second->SetTotalStartDelay(dDelay);
	if (0 != nRetVal)
	{
		return -3;
	}
...
double CBoard::GetTotalStartDelay(BYTE byControllerIndex)
{
	if (DCM_MAX_CONTROLLERS_PRE_BOARD <= byControllerIndex)
	{
		return 1e16 - 1;
	...
	return iterController->second->GetTotalStartDelay();
}

int CBoard::SetIODelay(USHORT usChannel, double dData, double dDataEn, double dHigh, double dLow)
{
	if (DCM_MAX_CHANNELS_PER_BOARD <= usChannel)
	{
		return -1;
	}
	auto iterController = m_mapController.find(usChannel / DCM_CHANNELS_PER_CONTROL);
	if (m_mapController.end() == iterController)
	{
		return -2;
	}
	int nRetVal = iterController->second->SetIODelay(usChannel % DCM_CHANNELS_PER_CONTROL, dData, dDataEn, dHigh, dLow);
	if (0 != nRetVal)
	{
...
int CBoard::GetIODelay(USHORT usChannel, double* pdData, double* pdDataEn, double* pdHigh, double* pdLow)
{
	if (DCM_MAX_CHANNELS_PER_BOARD <= usChannel)
	{
		return -1;
	...
	int nRetVal = iterController->second->GetIODelay(usChannel % DCM_CHANNELS_PER_CONTROL, pdData, pdDataEn, pdHigh, pdLow);
	if (0 != nRetVal)
	{
...
int CBoard::SaveDelay()
{
	CBoardDelay BoardDelay(m_bySlotNo);
	int nRetVal = BoardDelay.SaveDelay();
	if (0 != nRetVal)
	{
		switch (nRetVal)
		{
...
int CBoard::GetEdge(USHORT usChannel, BYTE byTimesetIndex, double* pdEdge, WAVE_FORMAT& WaveFormat, IO_FORMAT& IOFormat, COMPARE_MODE& CompareMode)
{
	...
	int nRetVal = iterController->second->GetEdge(usChannel % DCM_CHANNELS_PER_CONTROL, byTimesetIndex, pdEdge, WaveFormat, IOFormat, CompareMode);
	if (0 != nRetVal)
	{
		switch (nRetVal)
		{
	...
	USHORT usOperand, BOOL bCapture, BOOL bSwitch)
{
	...
	int nRetVal = iterController->second->SetVector(usChannel % DCM_CHANNELS_PER_CONTROL, bBRAM, uPatternLine, cPatternSign, byTimeset, lpszCMD, lpszParallelCMD, usOperand, bCapture, bSwitch);
	if (0 != nRetVal)
	{
		switch (nRetVal)
		{
...
int CBoard::SetVector(USHORT usChannel, UINT uPatternLine, char cPatternSign, USHORT usTimeset, std::set<CMD_INFO>& setCMD)
{
	...
	int nRetVal = iterController->second->AddChannelPattern(usChannel % DCM400_CHANNELS_PER_CONTROL, uPatternLine, cPatternSign, usTimeset, setCMD);
	if (0 != nRetVal)
	{
		/**
	 ...
	 * - -5 The operand of pattern command is over range
	 * - -6 The operand of vector command is over range
	 * - -7 The command is not support
		*/
		switch (nRetVal)
		{
		case -1:
		...
		case -4:
			...
			break;
		case -5:
			///<The operand of pattern command is over range
			nRetVal = -6;
			break;
		case -6:
			///<The operand of pattern command is over range
...
int CBoard::GetChannelMode(USHORT usChannel)
{
	...
	int nRetVal = iterController->second->GetChannelMode(usChannel % DCM_CHANNELS_PER_CONTROL);
	if (0 > nRetVal)
	{
...
int CBoard::GetChannelStatus(USHORT usChannel)
{
	...
	return iterController->second->GetChannelStatus(usChannel % DCM_CHANNELS_PER_CONTROL);
}
...
{
	m_setFailLineHandled.clear();
	m_setLatestFailSavingSelected = m_setFailSavingSelected;
	m_ChannelClassifiy.SetChannel(vecChannel);
	vector<USHORT> vecControllerChannel;
...
int CBoard::GetChannelResult(USHORT usChannel)
{
	...
	int nRetVal = iterController->second->GetChannelResult(usChannel % DCM_CHANNELS_PER_CONTROL);
	if (0 > nRetVal)
	{
		switch (nRetVal)
		{
...
int CBoard::GetFailCount(USHORT usChannel)
{
	...
	int nRetVal = iterController->second->GetFailCount(usChannel % DCM_CHANNELS_PER_CONTROL);
	if (0 > nRetVal)
	{
		switch (nRetVal)
		{
...
int CBoard::GetControllerFailCount(USHORT usChannel)
{
	if (DCM_MAX_CHANNELS_PER_BOARD <= usChannel)
	{
		return -1;
	...
	int nRetVal = iterController->second->GetFailCount();
	if (0 > nRetVal)
	{
		switch (nRetVal)
		{
...
int CBoard::GetFailLineNo(USHORT usChannel, UINT uGetMaxFailCount, std::vector<int>& vecBRAMLineNo, std::vector<int>& vecDRAMLineNo, BOOL bForceRefresh)
{
	...
	auto iterControl = m_mapController.find(usChannel / DCM_CHANNELS_PER_CONTROL);
	if (m_mapController.end() == iterControl || nullptr == iterControl->second)
	{
		return -2;
	}
	int nRetVal = iterControl->second->GetFailLineNo(usChannel % DCM_CHANNELS_PER_CONTROL, uGetMaxFailCount, vecBRAMLineNo, vecDRAMLineNo, bForceRefresh);
	if (0 > nRetVal)
	{
		switch (nRetVal)
		{
...
int CBoard::IsFailLineHandled(USHORT usChannel)
{
	if (DCM_MAX_CHANNELS_PER_BOARD <= usChannel)
	{
		return -1;
	...
	auto iterHandled = m_setFailLineHandled.find(byControllerIndex);
	if (m_setFailLineHandled.end() == iterHandled)
	{
		return -3;
	}
	return *iterHandled;
...
int CBoard::DeleteControllerFailLine(USHORT usChannel, int nBRAMDeleteCount, int nDRAMDeleteCount)
{
	...
	m_setFailLineHandled.insert(byControllerIndex);
	return 0;
}
...
int CBoard::GetLastCertainResultLineNo(USHORT usChannel, int& nBRAMLineNo, BOOL& bBRAMLineFail, int& nDRAMLineNo, BOOL& bDRAMLineFail)
{
	...
	iterControl->second->GetLastCertainResultLineNo(usChannel % DCM_CHANNELS_PER_CONTROL, nBRAMLineNo, bBRAMLineFail, nDRAMLineNo, bDRAMLineFail);
...
int CBoard::GetCapture(USHORT usChannel, std::vector<LINE_DATA>& vecBRAM, std::vector<LINE_DATA>& vecDRAM)
{
	...
	int nRetVal = iterController->second->GetCapture(usChannel % DCM_CHANNELS_PER_CONTROL, vecBRAM, vecDRAM);
	if (0 != nRetVal)
	{
		switch (nRetVal)
		{
...
int CBoard::GetCalibrationInfo(USHORT usChannel, STS_CALINFO &CalibrationInfo)
{
	...
	int nRetVal = iterController->second->GetCalibrationInfo(usChannel % DCM_CHANNELS_PER_CONTROL, CalibrationInfo);
	if (0 != nRetVal)
	{
		nRetVal = -3;
	}
...
int CBoard::SetCalibrationData(BYTE byControllerIndex, CAL_DATA *pCalibrationData, int nElementCount)
{
	if (DCM_MAX_CONTROLLERS_PRE_BOARD <= byControllerIndex)
	{
		return -1;
...
int CBoard::GetCalibrationData(BYTE byControllerIndex, CAL_DATA *pCalibrationData, int nElementCount)
{
	if (DCM_MAX_CONTROLLERS_PRE_BOARD <= byControllerIndex)
	{
		return -1;
...
int CBoard::GetChannelCount(BOOL bForceRead)
{
	...
	m_usChannelCount = byControllerCount * DCM_CHANNELS_PER_CONTROL;
...
int CBoard::SetChannelCount(USHORT usChannelCount)
{
	...
	if (DCM_MAX_CHANNELS_PER_BOARD < usChannelCount)
	{
		return -2;
	}
	BYTE byControllerCount = usChannelCount / DCM_CHANNELS_PER_CONTROL;
...
int CBoard::GetDynamicLoad(USHORT usChannel, BOOL& bEnable, double& dIOH, double& dIOL)
{
	...
	int nRetVal = 0;
	nRetVal = iterController->second->GetDynamicLoad(usChannel % DCM_CHANNELS_PER_CONTROL, bEnable, dIOH, dIOL);
...
void CBoard::GetPMUClampChannel(const std::vector<USHORT>& vecChannel, map<USHORT, UCHAR>& mapClampChannel)
{
	...
	for (auto& Controller : m_mapController)
	{
		...
		{
			USHORT usChannelOffset = DCM_CHANNELS_PER_CONTROL * Controller.first;
...
int CBoard::EnablePMUClampFlag(USHORT usChannel, BOOL bEnable)
{
	...
	vecChannel.push_back(usChannel % DCM_CHANNELS_PER_CONTROL);
...
double CBoard::GetPMUMeasureResult(USHORT usChannel, int nSampleTimes)
{
	if (DCM_MAX_CHANNELS_PER_BOARD < usChannel)
	{
		...
		m_pAlarm->SetAlarmMsg("The channel(S%d_%d) is over rang[0, %d].", m_bySlotNo, usChannel, DCM_MAX_CHANNELS_PER_BOARD - 1);
	...
	return iterController->second->GetPMUMeasureResult(usChannel % DCM_CHANNELS_PER_CONTROL, nSampleTimes);
}
...
double CBoard::GetPMUMode(USHORT usChannel, PMU_MODE& PMUMode, PMU_IRANGE& PMURange)
{
	...
	double dValue = iterController->second->GetPMUMode(usChannel % DCM_CHANNELS_PER_CONTROL, PMUMode, PMURange);
	if (1e10 < dValue)
	{
		return MAX_MEASURE_VALUE;
	}
...
int CBoard::GetVTMode(USHORT usChannel, VT_MODE& VTMode)
{
	...
	int nRetVal = 0;
	nRetVal = iterController->second->GetVTMode(usChannel % DCM_CHANNELS_PER_CONTROL, VTMode);
...
int CBoard::SetPrereadLine(const std::vector<USHORT>& vecChannel, UINT uStartLine, UINT uLineCount, MEM_TYPE MemType)
{
	set<BYTE> setControllerIndex;
	m_ChannelClassifiy.GetController(vecChannel, setControllerIndex);
	BOOL bNoBoard = TRUE;
	...
	for (auto Controller : setControllerIndex)
	{
		...
		bNoBoard = FALSE;
		nRetVal = iterController->second->SetPrereadLine(uStartLine, uLineCount, MemType);
		if (0 != nRetVal)
		{
			switch (nRetVal)
			{
...
int CBoard::SetLineInfo(const std::vector<USHORT>& vecChannel, UINT uStartLine, UINT uLineCount, MEM_TYPE MemType)
{
	...
	for (auto Controller : setControllerIndex)
	{
		...
		bNoBoard = FALSE;
		nRetVal = iterController->second->SetLineInfo(uStartLine, uLineCount, MemType);
		if (0 != nRetVal)
		{
			switch (nRetVal)
			{
...
int CBoard::SetWaveData(const std::vector<USHORT>& vecChannel, MEM_TYPE MemType, const BYTE* pbyData)
{	
	int nRetVal = 0;
	BOOL bNoBoard = TRUE;
	m_ChannelClassifiy.SetChannel(vecChannel);
	...
	for (auto& Controller : m_mapController)
	{
		...
		bNoBoard = FALSE;
		nRetVal = Controller.second->SetWaveData(vecControllerChannel, MemType, pbyData);
		if (0 != nRetVal)
		{
			switch (nRetVal)
			{
...
int CBoard::SetMemory(USHORT usChannel, BOOL bRAM, DATA_TYPE DataType, UINT uStartLine, UINT uLineCount, BYTE* pbyData)
{
	...
	int nRetVal = iterController->second->SetMemory(usChannel % DCM_CHANNELS_PER_CONTROL, bRAM, DataType, uStartLine, uLineCount, pbyData);
	if (0 != nRetVal)
	{
		switch (nRetVal)
		{
...
int CBoard::GetVector(USHORT usChannel, BOOL bBRAM, UINT uStartLine, UINT uLineCount, char* lpszPattern)
{
	...
	USHORT usByteChannel = usChannel % DCM_CHANNELS_PER_CONTROL;
...
double CBoard::GetPinLevel(USHORT usChannel, LEVEL_TYPE LevelType)
{
	...
	double dValue = iterController->second->GetPinLevel(usChannel % DCM_CHANNELS_PER_CONTROL, LevelType);
	if (10000 < dValue)
	{
		dValue = 1e16;
	}
...
int CBoard::SetTMUUnitChannel(std::vector<USHORT> vecChannel, BYTE byUnitIndex)
{
	...
	for (auto& Controller : m_mapController)
	{
		...
		bNoBoard = FALSE;
		nRetVal = Controller.second->SetTMUUnitChannel(vecControllerChannel[0] % DCM_CHANNELS_PER_CONTROL, byUnitIndex);
		if (0 != nRetVal)
		{
			switch (nRetVal)
			{
...
int CBoard::GetTMUConnectUnit(USHORT usChannel)
{
	...
	int nRetVal = iterController->second->GetTMUConnectUnit(usChannel % DCM_CHANNELS_PER_CONTROL);
	if (0 > nRetVal)
	{
		switch (nRetVal)
		{
...
int CBoard::GetTMUParameter(USHORT usChannel, BOOL& bRaiseTriggerEdge, USHORT& usHoldOffTime, USHORT& usHoldOffNum)
{
	...
	int nRetVal = iterController->second->GetTMUParameter(usChannel % DCM_CHANNELS_PER_CONTROL, bRaiseTriggerEdge, usHoldOffTime, usHoldOffNum);
	if (0 != nRetVal)
	{
		switch (nRetVal)
		{
...
int CBoard::GetTMUMeasure(USHORT usChannel, TMU_MEAS_MODE& MeasMode, UINT& uSampleNum, double& dTimeout)
{
	...
	int nRetVal = iterController->second->GetTMUMeasure(usChannel % DCM_CHANNELS_PER_CONTROL, MeasMode, uSampleNum, dTimeout);
	if (0 != nRetVal)
	{
		switch (nRetVal)
		{
...
double CBoard::GetTMUMeasureResult(USHORT usChannel, TMU_MEAS_TYPE MeasType, int& nErrorCode)
{
	...
	double dMeasResult = iterController->second->GetTMUMeasureResult(usChannel % DCM_CHANNELS_PER_CONTROL, MeasType, nErrorCode);
	if (0 != nErrorCode)
	{
		switch (nErrorCode)
		{
...
int CBoard::SetTriggerOut(USHORT usChannel)
{
	...
	int nRetVal = iterController->second->SetTriggerOut(usChannel % DCM_CHANNELS_PER_CONTROL);
	if (0 != nRetVal)
	{
		///<Not will happened
	}
...
int CBoard::SetFailSyn(const std::vector<CChannelGroup>& vecFailSynChannel, int& nConfictIndex)
{
	...
	for (const auto& ChannelGroup : vecFailSynChannel)
	{
		...
		setSynController.insert(bySynController);
		++nIndex;
	}
	if (bSynControllerConfict)
	{
...
int CBoard::EnableSaveSelectedFail(const std::vector<USHORT>& vecChannel, BOOL bEnable)
{
	...
	for (auto Controller : setController)
	{
		...
		if (m_mapController.end() != iterController && nullptr != iterController->second)
		{
			...
			auto iter = m_setFailSavingSelected.find(Controller);
			if (m_setFailSavingSelected.end() == iter)
			{
				if (bEnable)
				{
					m_setFailSavingSelected.insert(Controller);
				}
			}
			else if(!bEnable)
			{
				m_setFailSavingSelected.erase(iter);
			}
		}
	}
	if (bNoBoard)
...
BOOL CBoard::IsSavingSelectedFail(USHORT usChannel) const
{
	if (DCM_MAX_CHANNELS_PER_BOARD <= usChannel)
	{
		return -1;
	...
	BYTE byController = usChannel / DCM_CHANNELS_PER_CONTROL;
	if (m_mapController.end() == m_mapController.find(byController))
	{
		return -2;
	}
	if (m_setLatestFailSavingSelected.end() != m_setLatestFailSavingSelected.find(byController))
	{
		return 1;
	}
	return 0;
...
void CBoard::InitController()
{
	...
	BYTE byControllerCount = m_usChannelCount / DCM_CHANNELS_PER_CONTROL;
	auto iterController = m_mapController.begin();
	...
	for (;byControllerIndex < DCM_MAX_CONTROLLERS_PRE_BOARD;++byControllerIndex)
	{
		iterController = m_mapController.find(byControllerIndex);
		if (m_mapController.end() != iterController)
		{
...
void CBoard::SetDelay()
{
	int nRetVal = 0;
	double dTimesetDelay = 0;
	double dTotalStartDelay = 0;
	CBoardDelay::IO_DELAY IODelay;
	CBoardDelay BoardDelay(m_bySlotNo);
	...
	for (auto& Controller : m_mapController)
	{
		...
		nRetVal = BoardDelay.ReadDelay(Controller.first);
		if (0 != nRetVal)
		{
			continue;
		}

		BoardDelay.GetControllerDelay(&dTimesetDelay, &dTotalStartDelay);
		Controller.second->SetTimesetDelay(dTimesetDelay);
		Controller.second->SetTotalStartDelay(dTotalStartDelay);
		for (USHORT usChannel = 0; usChannel < DCM_CHANNELS_PER_CONTROL;++usChannel)
		{
			BoardDelay.GetIODelay(usChannel, &IODelay);
			Controller.second->SetIODelay(usChannel, IODelay.m_dData, IODelay.m_dDataEn, IODelay.m_dHigh, IODelay.m_dLow);
		}
		Controller.second->UpdateDelay();
	}
}
